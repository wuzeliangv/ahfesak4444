 ...
 * Traffic query modal.
 *
 * Pulls NetworkIn + NetworkOut totals from CloudWatch for a single EC2
 * instance over a user-selected date range. Defaults to
 * `launch_time → today` so the modal is usable in one click for the common
 * "lifetime traffic" question, while still letting the user pick custom
 * windows for billing reconciliation.
 *
 * Date inputs are HTML `type=date` (YYYY-MM-DD). We convert them to UTC
 * ISO-8601 windows: start = 00:00:00Z of the start day, end = 23:59:59Z of
 * the end day, so a single-day query covers the whole day in UTC.
 */
...
import { api, type Ec2Instance, type Ec2TrafficData } from '@/lib/api';
...
/** Format a Date as YYYY-MM-DD in local time, suitable for `<input type="date">`. */
...
/** Extract YYYY-MM-DD (UTC) from an ISO timestamp; null if invalid. */
...
export function TrafficModal({ open, instance, accountId, onClose }: Props) {
  const today = useMemo(() => toDateInputValue(new Date()), []);
  const launchDate = useMemo(() => isoToUtcDate(instance?.launch_time), [instance?.launch_time]);

  const [startDate, setStartDate] = useState<string>(launchDate ?? today);
  const [endDate, setEndDate] = useState<string>(today);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<Ec2TrafficData | null>(null);

  // Reset state every time a new instance is opened.
  useEffect(() => {
    if (!open) return;
    ...
    setError(null);
    setResult(null);
  }, [open, launchDate, today, instance?.instance_id]);

  const mutation = useMutation({
    mutationFn: async (vars: { start: string; end: string }) => {
      if (!instance) throw new Error('未选择实例');
      const creds = await getAccountCredentials(accountId);
      // 00:00:00Z of start day → 23:59:59Z of end day (inclusive of full UTC days).
      const startIso = `${vars.start}T00:00:00.000Z`;
      const endIso = `${vars.end}T23:59:59.999Z`;
      return api.ec2Traffic(creds, instance.region, instance.instance_id, startIso, endIso);
    },
    onSuccess: (data) => {
      setResult(data);
      setError(null);
  ...
  // Auto-fire the query once when the modal first opens for an instance.
  useEffect(() => {
    if (!open || !instance) return;
    // Only auto-query if we have a sensible default range.
        ...
        instance ? (
          <span>
            ...
            <span className="ml-2 text-[var(--color-fg-muted)]">
              · {instance.region} · {instance.instance_id}
            </span>
          </span>
        ) : undefined
      }
